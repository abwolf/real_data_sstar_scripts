...
tag <- ARGV[1]
neand.callable.file <- ARGV[2]
recomb_rates_file <- ARGV[3]
outputdir <- ARGV[4]
nullmode <- ARGV[5]
sstar_null_model<- ARGV[6]
region_starts <- ARGV[7]
region_chrs <- ARGV[8]
refname <- ARGV[9]
n_req_window_snps <- as.numeric(ARGV[10])
spval <- 0.01
matchpval <- 0.05
...
dat.allchr = fread(input = paste0( 'zcat ',windowcalcfile), select = colstowrite, header=TRUE)

if (nrow(dat.allchr)==0){
    stop("S* output file empty...Exiting")
}
...
dat <- dat.allchr %>% filter(chrom==achr)
